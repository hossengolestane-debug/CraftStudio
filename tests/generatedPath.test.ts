...
describe('generated path safety', () => {
  ...
  it('only allowlists trusted output paths', () => {
    ...
    expect(isTrustedOutputPath('CONFIG.md')).toBe(true)
    expect(isTrustedOutputPath('DATAPACK.md')).toBe(true)
    expect(isTrustedOutputPath('river_stones.config.json')).toBe(true)
    expect(isTrustedOutputPath('datapack/pack.mcmeta')).toBe(true)
