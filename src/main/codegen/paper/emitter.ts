...
import { paperPinsFor } from '../../../shared/platformPins'
...
function itemFactory(spec: ProjectSpec): string {
  return spec.items
    ...
    meta.lore(java.util.List.of(Component.text("CraftStudio custom item (vanilla paper + PDC)")));
...
function paperMain(spec: ProjectSpec): string {
...
${commandSwitch(spec)}
    ...
    player.sendMessage("Gave " + id + " (vanilla paper with PDC). A resource pack is not included.");
    return true;
  }
}
`
...
`
  ...
  files.push({
    ...
    contents: [
      ...
      '5. Clients see a renamed vanilla paper item unless they add their own resource pack.',
      '',
  ...
  files.push({
    ...
    contents: [
      ...
      '',
      '**Client limit:** Paper cannot register a new client item id. Players see vanilla paper with a display name and persistent data. Resource-pack export is not implemented.',
      '',
  ...
  files.push({
    ...
    contents: [
      ...
      'Custom items use existing Material.PAPER + PersistentDataContainer. No new client item type.',
      '',
      '```bash',
      './gradlew build',
      '```',
